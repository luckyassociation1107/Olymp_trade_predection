...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if uid == ADMIN_ID:
        if not is_logged:
            msg = "👋 Admin! `/setpass 123` or `/login pass`"
        ...
        status = "✅ VIP ACTIVE" if is_user_vip else "🆓 FREE MEMBER"
        await update.message.reply_text(f"Status: {status}\nPredictions arrive automatically.")
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await q.answer()
    if q.data == 'buy_vip':
        if os.path.exists(QR_PATH): await context.bot.send_photo(chat_id=uid, photo=open(QR_PATH, 'rb'), caption="Send screenshot after payment.")
